...
public class DocumentBenchmarking {
	...
	public static void main(String [] args) {
				...
				numToCheck += increment)
		{
			...
			for (int numTrials = 1; numTrials <= trials; numTrials++) {
				BasicDocument basicDoc = new BasicDocument(numChars);
				basicDoc.getFleschScore();
			}
			endTime = System.nanoTime();
			basicTime = (long) ((endTime - startTime)/1000000000.0);
			System.out.print(basicTime + "\t" );
			startTime = System.nanoTime(); 
			for (int numTrials = 1; numTrials <= trials; numTrials++) {
				EfficientDocument effDoc = new EfficientDocument(numChars);
				effDoc.getFleschScore();
			}
			endTime = System.nanoTime();
			efficientTime = (endTime - startTime)/1000000000;
